 ...
 *
 * Runs client-side boundary crossing checks when device coordinates poll.
 */
export function useGeofenceDetection() {
    const { accessToken } = useAuthStore();
    const { devices } = useDeviceStore();
    const { geofences: localGeofences, apiGeofences, fetchApiGeofences } = useGeofenceStore();
    
    // Fetch API geofences once access token and devices are available
    const fetchedRef = useRef(false);
    useEffect(() => {
        if (accessToken && (!fetchedRef.current || (devices.length > 0 && !fetchedRef.current))) {
            fetchApiGeofences();
            if (devices.length > 0) {
    ...
    useEffect(() => {
        if (!combinedGeofences.length || !devices.length) return;
        ...
        devices.forEach((device: Device) => {
            ...
            combinedGeofences.forEach((fence: LocalGeofence) => {
                ...
                if (isInside && !wasInside) {
                    ...
                    // Only fire if alarm type allows "in"
                    ...
                    // Only fire if alarm type allows "out"
    ...
    }, [devices, combinedGeofences, addEvent]);
}
