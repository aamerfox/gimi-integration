...
async function main() {
    ...
    const devs = devRes.result || [];
    const locs = locRes.result || [];

    console.log(`\n--- DEVICE NAME COMPARISON ---`);
    for (const dev of devs) {
        const loc = locs.find(l => l.imei === dev.imei);
        const name1 = dev.deviceName;
        const name2 = loc ? loc.deviceName : 'NOT_FOUND';
        console.log(`IMEI: ${dev.imei} | DeviceList: '${name1}' | LocationList: '${name2}' | Match: ${name1 === name2}`);
    }
}
